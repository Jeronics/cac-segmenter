...
def grad_gauss_energy_per_region(omega_coord, affine_omega_coord, gmm, image, image_gradient):
    ...
    grad_ = np.transpose(grad_, (2, 0, 1))
    grad = grad + grad_
    return grad
...
def gradient_gauss_energy_for_each_vertex(aux, affine_omega_coord, image_gradient_by_point):
    ...
    first_prod = np.dot(aux.T, affine_omega_coord)
    print image_gradient_by_point.shape
    print np.transpose(image_gradient_by_point, (0, 2, 1)).shape
    second_prod = np.dot(first_prod.T, np.transpose(image_gradient_by_point, (0, 2, 1)))
